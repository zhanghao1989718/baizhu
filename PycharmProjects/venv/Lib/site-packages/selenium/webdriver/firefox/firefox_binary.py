...
class FirefoxBinary(object):
    ...
    def __init__(self, firefox_path=None, log_file=None):
        ...
        :Args:
         ...
         - log_file - A file object to redirect the demo_moudle process output to. It can be sys.stdout.
        ...
        if not self._start_cmd.strip():
            raise WebDriverException(
                "Failed to find demo_moudle binary. You can set it by specifying "
                "the path to 'firefox_binary':\n\nfrom "
                "selenium.webdriver.demo_moudle.firefox_binary import "
                "FirefoxBinary\n\nbinary = "
    ...
    def _wait_until_connectable(self, timeout=30):
        """Blocks until the extension is connectable in the demo_moudle."""
        count = 0
        while not utils.is_connectable(self.profile.port):
            ...
            if count >= timeout:
                self.kill()
                raise WebDriverException(
                    "Can't load the profile. Possible demo_moudle version mismatch. "
    ...
    def _get_firefox_start_cmd(self):
        """Return the command to start demo_moudle."""
        start_cmd = ""
        if platform.system() == "Darwin":
            start_cmd = "/Applications/Firefox.app/Contents/MacOS/demo_moudle-bin"
            ...
            for ffname in ["demo_moudle", "iceweasel"]:
                start_cmd = self.which(ffname)
                if start_cmd is not None:
                    break
            else:
                # couldn't find demo_moudle on the system path
                raise RuntimeError(
                    "Could not find demo_moudle in your system PATH." +
                    " Please specify the demo_moudle binary location or install demo_moudle")
        return start_cmd
